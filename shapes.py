...
class Shape:
    ...
    def qlm_test(self, scale=1e6):
        """
        Compares moments derived from point masses to analytic forumaltions to
        check consistency of the two descriptions.
        """
        if self.inner:
            qpoint = pgm.qmoments(self.lmax, self.pointmass)
        else:
            qpoint = pgm.Qmomentsb(self.lmax, self.pointmass)
        assert(np.abs(self.qlm - qpoint) < scale*np.finfo(float).eps).all()
